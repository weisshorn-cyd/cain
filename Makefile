...
CAIN_CHART           ?= $(REGISTRY)/$(GITHUB_REPOSITORY)-chart
...
publish-chart: build-chart helm
	$(HELM) push cain-$(CHART_VERSION).tgz oci://$(CAIN_CHART)
...
golangci-lint: $(LOCALBIN)
	...
	curl -sSfL https://golangci-lint.run/install.sh | sh -s -- -b $(LOCALBIN) v$(GOLANGCI_LINT_VERSION)
